    ...
    <div class="apiDetails">
      ...
      &lt;!&ndash;        <li :class="{'active':index===1}" @click="index=1" v-show="info.sysyemNames.length">接口调试</li>&ndash;&gt;
            ...
            <div class="details" v-if="!index">
                ...
                <div class="listInfo" v-show="ipWhiteList.length" v-if="info.sysyemNames.length">
                    <p class="title">IP白名单</p>
                    <a-table
                            :columns="columns"
                ...
                <div class="listInfo" v-show="ipBlackList.length" v-if="info.sysyemNames.length">
                    <p class="title">IP黑名单</p>
                    <a-table
                            :columns="columns"
                            :data-source="ipBlackList"
                            class="apiDetails_tabel"
                            bordered
                            :pagination="{hideOnSinglePage:true}"
                            :rowKey="row=>row.index"
        ...
        <span>
          ...
          <a-select-option value="3">
            接口网络
                ...
                <div v-show="monitoringType!=='2'" style="margin-right: 20px;padding-top: 4px">
                    订阅系统：{{systemName}}
                </div>
                <div class="timer">
                    <span>时间范围：</span>
        ...
        methods: {
            ...
            getApiDetails() {//api详情
                this.axios.get("/publishApi/" + this.$route.query.id).then(res => {
                    ...
                    if (this.info.ipWhiteList) {
                        this.info.ipWhiteList.forEach((item, index) => {
                            this.ipWhiteList.push({
                                index: index + 1,
                                ip: item
                            })
                        })
                    } else if (this.info.ipBlackList) {
                        this.info.ipBlackList.forEach((item, index) => {
            ...
            getApiParameter() {
                ...
                let flag;
                if (this.routerList[this.active || 0].responseBody.dataType===null&&this.routerList[this.active || 0].responseBody.defaultValue===null&&this.routerList[this.active || 0].responseBody.object===null&&this.routerList[this.active || 0].responseBody.description===null){
                    flag=false;
                }else {
                    flag=true;
                }
                let responseBody = {
                    name: "responseBody",
                    type: this.routerList[this.active || 0].responseBody.dataType || "-",
                    defaultValue: this.routerList[this.active || 0].responseBody.defaultValue || "-",
                    value: this.routerList[this.active || 0].responseBody.description || "-",
                    index: 0,
                    flag:flag
                }
                if (this.routerList[this.active || 0].responseBody.object) {
            ...
            getSystemList() {
                this.axios.get(`/analytics/getSubscribeSystems/${this.$route.query.id}/${this.$route.query.listId}`).then(res => {
                    if (res.code==="0"){
                        console.log(res.data);
                        this.systemId=res.data.appId;
            ...
            fileClick(item){
                if (window.location.host!=="portal-web-hisense-apigateway-test.devapps.hisense.com"&&window.location.host!=="localhost:8080"){
                    if (item.fileName.substring(item.fileName.length-3,item.fileName.length)==="jar"){
                        ...
                        window.open("http://kkfileview-hisense-hip-prd.prdapp.hisense.com/onlinePreview?url="+encodeURIComponent(previewUrl));
                    }
                }else{
                    if (item.fileName.substring(item.fileName.length-3,item.fileName.length)==="jar"){
